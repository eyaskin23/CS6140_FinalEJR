...
if __name__ == '__main__':
    ...
    #tells the pc to use the GPU if one is available, otherwise simply runs from cpu
    use_gpu = torch.cuda.is_available()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    tensor, label = input_array[0]
    input_size = tensor.shape[0]
    #runs through all the multiples of 32 I for the hidden layer size
    for mult in range(1,7):
        ...
        for fold, (train_ids, test_ids) in enumerate(kf.split(range(len(input_array)))):
            total_loss = 0
            count = 0
            # this portion calls the file which combines the hidden layer and output layer into a single workflow nn.Module that can
            # be called and have its weights and biases modified by the training loop below
            ...
            train_loader = DataLoader(input_array, batch_size=64, sampler = train_subsample, num_workers=4, pin_memory=use_gpu, persistent_workers=True)
            test_loader = DataLoader(input_array, batch_size=64, sampler = test_subsample, num_workers=4, pin_memory=use_gpu, persistent_workers=True)
            ...
            for epochs in range(num_epochs):
                ...
                for tensor, label in train_loader:
                    ...
                    t_targets = label
                    #this zeros any current gradients calculated by the optimizer
                    optimizer.zero_grad()
            ...
            train_final_accuracies.append(accuracy)
            ...
            test_final_accuracies.append(accuracy)
...
#       #This generates the combined confusion matrix for all folds used in a set of hyperparameters
